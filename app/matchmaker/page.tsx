...
function score(anchor: typeof POKEMON_LIST[0], candidate: typeof POKEMON_LIST[0]): number {
  if (anchor.habitat !== candidate.habitat) return -1; // hard filter
  const shared = sharedItemCount(anchor.categories, candidate.categories);
  // Complementary specialty bonus (+50% if specialties differ)
...
function buildGroup(anchor: typeof POKEMON_LIST[0], size: number): typeof POKEMON_LIST {
  ...
  while (group.length < size && candidates.length > 0) {
    // Score each candidate against ALL current group members
    const groupCats = [...new Set(group.flatMap((m) => m.categories))];
    const fakeAnchor = { ...anchor, categories: groupCats };
    let bestIdx = 0;
    let bestScore = -1;
  ...
  return (
    <div className="detail-wrap">
      ...
      <div className="card">
        ...
        {anchor && (
          <div className="pkmn-head" style={{ marginTop: 20, borderBottom: "none", paddingBottom: 0 }}>
            ...
            <div className="pkmn-info">
              <div className="pkmn-num">Anchor · #{String(anchor.num).padStart(3, "0")}</div>
          ...
          {recommendations.map(({ pokemon: p, score: s, shared }) => {
            ...
            return (
              ...
              >
                ...
                <div className="best-item-icon" style={{ width: 56, height: 56 }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="best-item-body">
                  <div className="best-item-name">
                    <Link href={`/pokemon/${p.slug}`} style={{ color: "inherit", textDecoration: "none" }} onClick={(e) => e.stopPropagation()}>
                      {p.name}
                    </Link>
                  </div>
        ...
        <div className="card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {[{ p: anchor, label: "Anchor" }, { p: compareTarget, label: "Candidate" }].map(({ p, label }) => (
              <div key={p.slug}>
                <div className="detail-meta" style={{ marginBottom: 8 }}>{label}: {p.name}</div>
          ...
          {compareShared.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <div className="stat-label" style={{ marginBottom: 8 }}>Shared items</div>
      ...
      {anchor && group.length > 1 && (
        <div className="card">
          ...
          <div className="pkmn-grid" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))" }}>
            {group.map((p, i) => (
              <Link key={p.slug} href={`/pokemon/${p.slug}`} className="pkmn-grid-card">
                {i === 0 && <div style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "var(--accent)", marginBottom: 4 }}>ANCHOR</div>}
