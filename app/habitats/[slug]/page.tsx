...
export default async function HabitatPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const resolved: ResolvedReq[] = requirements.map((req) => {
    ...
    // Strip trailing " (any)" to get the base type name for matching
    ...
    //    Catches both "Log bed" (ends with) and "Straw Table" (starts with)
        ...
        <Card>
          ...
          <div className="flex flex-wrap gap-3 mt-3">
            {resolved.map(({ req, item, isExact, isAny }) => {
              ...
              return (isExact && item)
                ? <Link key={req.name} href={`/item/${item.slug}`} className="no-underline">{inner}</Link>
